...
<div class="container">
    ...
    <div style="position: absolute;margin-left: 900px;margin-top: -40px;">
...
<script language=javascript>
        ...
        if(document.getElementById('comment_reply_' + id).style.display == 'none'){
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    var okHtml;
                    if(list != null && list != ""){
                        // 将其结果拼装在 div 中，其中 okHtml(chml) 代表的是一级评论列表，oHtml 是二级评论列表（或者说子评论列表），一个一级评论可能有多个子评论。
                        // 我们需要判断该一级评论是否有子评论，如果有就对子评论列表集合进行遍历，获取每个子评论。ehtml 是 div 结束标签。
                        $(list).each(function(){
                            ...
                            if(this.children != null && this.children != '') {
                                ...
                                $(commentList).each(function () {
                                    ...
                                    var oHtml = "<div class='all-pl-con'><div class='pl-text hfpl-text clearfix'>" +
                                        "<a class='comment-size-name'>" + this.user.nickName + "</a><a class='atName'>@" + this.byUser.nickName + " :</a> " +
                                        "<span class='my-pl-con'>" + this.comContent + "</span>" +
                                        "</div><div class='date-dz'> <span class='date-dz-left pull-left comment-time'>" + FormatDate(this.commTime) + "</span>" +
                                        "<div class='date-dz-right pull-right comment-pl-block'>" +
                            ...
                            okHtml = okHtml + ehtml;
                            // 将评论回复列表 div 追加在当前文章的可添加评论内容的空 div下。
    ...
    function deleteComment(con_id,uid,id,fid){
        if('${user.id}' == uid){
                    ...
                    success:function(data){
                        ...
                        if(comm_data == "fail"){
                            ...
                            if(oT.siblings('.all-pl-con').length >= 1){
                                ...
                                oThis.parents('.date-dz-right').parents('.date-dz').parents('.all-pl-con').parents('.hf-list-con')
                                    .css('display','none');
                                oT.remove();
                            }
            ...
            window.alert("you perssion isn't enought!")
        }
    }
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if(comm_data == "fail"){
                        ...
                        '<img src="${user.imgUrl}" alt=""></div> <div class="comment-show-con-list pull-left clearfix"> '+
                        '<div class="pl-text clearfix"> <a  class="comment-size-name">${user.nickName} : </a> <span class="my-pl-con"> '+
                        ...
                        '<a style="cursor:pointer" onclick="comment_hf('+content_id+','+id+','+null+','+comm_data.user.id+','+cuid+')" '+
                        'id="comment_hf_'+id+'" class="date-dz-pl pl-hf hf-con-block pull-left">回复</a> <span class="pull-left date-dz-line">'+
                        '|</span> <a onclick="reply_up('+id+')" id="change_color_'+id+'" style="cursor:pointer"  class="date-dz-z pull-left" >'+
                        '<i class="date-dz-z-click-red"></i>赞 (<i class="z-num" id="comment_upvote_'+id+'">0</i>)</a> </div> </div> '+
    ...
    /**
     *
     * (1）upvote_click 函数接收两个参数：（1）文章 id；（2）常量1（赞）或者-1（踩）。
    ...
    function upvote_click(id,cont){
        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                if(up == "success"){
                    if(cont == -1){
                        var down = document.getElementById("dovn_"+id);
